...
/**
 * Configuration for ESP8266 GUIKit loader
 * Stored in /etc/guikitloader.conf
 */
const ConfigState = {
    default_gui: '',
    gui_path: '/gui',
...
async function loadConfig() {
    try {
        const response = await fetch('/etc/guikitloader.conf');
        if (response.ok) {
        ...
        logToConsole(`Config load error: ${error.message}`);
    }
    
    // Use defaults
    resetConfig();
    logToConsole('Using default configuration');
    return false;
}
...
function parseConfig(configText) {
    ...
    for (const line of lines) {
        ...
        switch (key) {
            case 'default_gui':
                ConfigState.default_gui = value;
                break;
            case 'gui_path':
                ConfigState.gui_path = value;
                break;
            case 'use_project_dirs':
                ConfigState.use_project_dirs = value.toLowerCase() === 'true';
                break;
            case 'auto_load_last':
                ConfigState.auto_load_last = value.toLowerCase() === 'true';
                break;
            case 'last_gui':
                ConfigState.last_gui = value;
                break;
        }
    }
}
...
function resetConfig() {
    ConfigState.default_gui = '';
    ConfigState.gui_path = '/gui';
    ConfigState.use_project_dirs = true;
    ConfigState.auto_load_last = true;
    ConfigState.last_gui = '';
}
...
function generateConfig() {
    return `# GUIKit Configuration
# Generated by GUIKit Web Editor
# This file configures which GUI is loaded by default on the ESP8266

default_gui=${ConfigState.default_gui}
gui_path=${ConfigState.gui_path}
use_project_dirs=${ConfigState.use_project_dirs}
auto_load_last=${ConfigState.auto_load_last}
last_gui=${ConfigState.last_gui}
`;
}

/**
 * Save configuration to ESP8266
 */
async function saveConfig() {
    try {
        ...
        if (response.ok) {
            logToConsole('Configuration saved to ESP8266');
            return true;
        } else {
            logToConsole('Error: Failed to save configuration');
            return false;
        }
    } catch (error) {
        logToConsole(`Config save error: ${error.message}`);
        return false;
    }
}

/**
 * Set a project as the default GUI to load on boot
 * @param {string} projectName - Project name (without .GUIKIT suffix)
 */
async function setAsDefaultGUI(projectName) {
    if (!projectName || projectName.trim() === '') {
        logToConsole('Error: Project name cannot be empty');
        return false;
    }
    ...
    ConfigState.default_gui = name;
    ConfigState.last_gui = name;
    
    const saved = await saveConfig();
    if (saved) {
        logToConsole(`Project "${name}" set as default GUI`);
        return true;
    }
    return false;
}
...
/**
 * Deploy current project to ESP8266
 * Saves all project files to the device
 */
async function deployProject() {
    if (!ProjectState.currentProject) {
        logToConsole('Error: No active project to deploy');
        return false;
    }
    ...
    try {
        // Save the current GUI state to the project first
        const saved = await saveProject();
        if (!saved) {
            logToConsole('Error: Failed to save project before deploy');
            return false;
        }
        
        // Deploy all files from the project to ESP8266
        // This would copy the entire .GUIKIT directory structure
        logToConsole(`Deploying project "${project.name}" to ${project.location}`);
        
        // For WebDAV deployment, files are already there
        // For local deployment, we need to upload to ESP8266
        if (project.location === 'webdav') {
            logToConsole('Project already on WebDAV/ESP8266');
        } else {
            // Local project - need to upload
            logToConsole('Local project deployment: Upload to /gui/ on ESP8266');
            // This would require file upload functionality
        }
        
        logToConsole(`Project "${project.name}" deployed successfully`);
        return true;
        
    } catch (error) {
        logToConsole(`Deploy error: ${error.message}`);
        return false;
    }
}
 ...
 */
async function deployAndSetDefault(projectName) {
    const deployed = await deployProject();
    if (!deployed) return false;
    
    return await setAsDefaultGUI(projectName);
}
...
window.setAsDefaultGUI = setAsDefaultGUI;
window.deployProject = deployProject;
window.deployAndSetDefault = deployAndSetDefault;
